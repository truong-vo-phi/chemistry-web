...
export const metadata: Metadata = {
  title: 'Quản Lý Nội Dung | ChemLab 3D',
  description: 'Quản trị cấu trúc nội dung, phiên bản và trạng thái xuất bản.',
};
    ...
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className="mx-auto w-full max-w-container-max px-margin-mobile py-8 md:px-margin-desktop">
        <section className="rounded-xl border-2 border-primary-fixed-dim bg-surface-container-lowest p-6 shadow-sm">
          ...
          <h1 className="font-display-lg-mobile text-display-lg-mobile text-primary md:font-headline-md md:text-headline-md">Quản Lý Nội Dung</h1>
          <p className="mt-3 max-w-3xl text-on-surface-variant">Quản trị cấu trúc nội dung, phiên bản và trạng thái xuất bản.</p>
          <p className="mt-2 text-xs text-outline">Route: {appRoutes.contentManagement}</p>

          <div className="mt-5 flex flex-wrap gap-3">
            <Link href={appRoutes.createContent} className="rounded-full border-2 border-on-primary-fixed-variant bg-primary px-5 py-2 font-label-sm text-on-primary shadow-[0_4px_0_0_rgba(0,67,149,0.3)]">
              Tạo nội dung mới
            </Link>
            ...
            <Link href={appRoutes.theoryCourseLibrary} className="rounded-full border-2 border-outline-variant bg-surface px-5 py-2 font-label-sm text-on-surface hover:bg-surface-container">
