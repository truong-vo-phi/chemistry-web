...
export const metadata: Metadata = {
  title: 'Tạo Nội Dung | ChemLab 3D',
  description: 'Tạo bài học, thí nghiệm, tài liệu và cấu phần tương tác theo chuẩn mới.',
};

const stats = [
  { icon: 'edit_note', label: 'Bản nháp', value: '18' },
            { icon: 'publish', label: 'Đã xuất bản', value: '42' },
            { icon: 'pending_actions', label: 'Chờ duyệt', value: '7' }
];
    ...
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className="mx-auto w-full max-w-container-max px-margin-mobile py-8 md:px-margin-desktop">
        <section className="rounded-xl border-2 border-primary-fixed-dim bg-surface-container-lowest p-6 shadow-sm">
          <p className="mb-2 text-xs font-bold uppercase tracking-wider text-on-surface-variant">Biên soạn/Nội dung</p>
          <h1 className="font-display-lg-mobile text-display-lg-mobile text-primary md:font-headline-md md:text-headline-md">Tạo Nội Dung</h1>
          <p className="mt-3 max-w-3xl text-on-surface-variant">Tạo bài học, thí nghiệm, tài liệu và cấu phần tương tác theo chuẩn mới.</p>
          <p className="mt-2 text-xs text-outline">Route: {appRoutes.createContent}</p>

          <div className="mt-5 flex flex-wrap gap-3">
            <Link href={appRoutes.contentManagement} className="rounded-full border-2 border-on-primary-fixed-variant bg-primary px-5 py-2 font-label-sm text-on-primary shadow-[0_4px_0_0_rgba(0,67,149,0.3)]">
              Về quản lý nội dung
            </Link>
            <Link href={appRoutes.courseBuilder} className="rounded-full border-2 border-outline-variant bg-surface px-5 py-2 font-label-sm text-on-surface hover:bg-surface-container">
              Mở course builder
            </Link>
            <Link href={appRoutes.lessonInterface} className="rounded-full border-2 border-outline-variant bg-surface px-5 py-2 font-label-sm text-on-surface hover:bg-surface-container">
              Xem giao diện bài học
            </Link>
          </div>
        </section>
