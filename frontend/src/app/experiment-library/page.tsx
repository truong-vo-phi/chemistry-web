...
export const metadata: Metadata = {
  title: 'Thư Viện Thí Nghiệm | ChemLab 3D',
  description: 'Kho thí nghiệm trực quan để bắt đầu thực hành theo độ khó và chủ đề.',
};
    ...
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className="mx-auto w-full max-w-container-max px-margin-mobile py-8 md:px-margin-desktop">
        <section className="rounded-xl border-2 border-primary-fixed-dim bg-surface-container-lowest p-6 shadow-sm">
          ...
          <h1 className="font-display-lg-mobile text-display-lg-mobile text-primary md:font-headline-md md:text-headline-md">Thư Viện Thí Nghiệm</h1>
          <p className="mt-3 max-w-3xl text-on-surface-variant">Kho thí nghiệm trực quan để bắt đầu thực hành theo độ khó và chủ đề.</p>
          <p className="mt-2 text-xs text-outline">Route: {appRoutes.experimentLibrary}</p>

          <div className="mt-5 flex flex-wrap gap-3">
            <Link href={appRoutes.launch3dLab} className="rounded-full border-2 border-on-primary-fixed-variant bg-primary px-5 py-2 font-label-sm text-on-primary shadow-[0_4px_0_0_rgba(0,67,149,0.3)]">
              Khởi chạy 3D Lab
            </Link>
            ...
            <Link href={appRoutes.experimentResults} className="rounded-full border-2 border-outline-variant bg-surface px-5 py-2 font-label-sm text-on-surface hover:bg-surface-container">
