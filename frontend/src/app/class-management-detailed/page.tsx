...
export const metadata: Metadata = {
  title: 'Quản Lý Lớp Học Chi Tiết | ChemLab 3D',
  description: 'Theo dõi lớp, tiến độ, hoạt động và phân công trong từng nhóm học viên.',
};

const stats = [
  { icon: 'groups', label: 'Người dùng', value: '1,248' },
            { icon: 'school', label: 'Lớp hoạt động', value: '36' },
            { icon: 'verified', label: 'Hệ thống', value: 'Ổn định' }
];
    ...
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className="mx-auto w-full max-w-container-max px-margin-mobile py-8 md:px-margin-desktop">
        <section className="rounded-xl border-2 border-primary-fixed-dim bg-surface-container-lowest p-6 shadow-sm">
          <p className="mb-2 text-xs font-bold uppercase tracking-wider text-on-surface-variant">Quản trị</p>
          <h1 className="font-display-lg-mobile text-display-lg-mobile text-primary md:font-headline-md md:text-headline-md">Quản Lý Lớp Học Chi Tiết</h1>
          <p className="mt-3 max-w-3xl text-on-surface-variant">Theo dõi lớp, tiến độ, hoạt động và phân công trong từng nhóm học viên.</p>
          <p className="mt-2 text-xs text-outline">Route: {appRoutes.classManagementDetailed}</p>

          <div className="mt-5 flex flex-wrap gap-3">
            <Link href={appRoutes.teacherDashboard} className="rounded-full border-2 border-on-primary-fixed-variant bg-primary px-5 py-2 font-label-sm text-on-primary shadow-[0_4px_0_0_rgba(0,67,149,0.3)]">
              Về Teacher Dashboard
            </Link>
            <Link href={appRoutes.userManagement} className="rounded-full border-2 border-outline-variant bg-surface px-5 py-2 font-label-sm text-on-surface hover:bg-surface-container">
              Quản lý người dùng
            </Link>
            <Link href={appRoutes.learningCommunity} className="rounded-full border-2 border-outline-variant bg-surface px-5 py-2 font-label-sm text-on-surface hover:bg-surface-container">
              Mở cộng đồng lớp
            </Link>
          </div>
        </section>
