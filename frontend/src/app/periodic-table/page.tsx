...
export const metadata: Metadata = {
  title: 'Bảng Tuần Hoàn | ChemLab 3D',
  description: 'Tra cứu nguyên tố và xu hướng hóa học trong giao diện học tập trực quan.',
};
    ...
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className="mx-auto w-full max-w-container-max px-margin-mobile py-8 md:px-margin-desktop">
        <section className="rounded-xl border-2 border-primary-fixed-dim bg-surface-container-lowest p-6 shadow-sm">
          ...
          <h1 className="font-display-lg-mobile text-display-lg-mobile text-primary md:font-headline-md md:text-headline-md">Bảng Tuần Hoàn</h1>
          <p className="mt-3 max-w-3xl text-on-surface-variant">Tra cứu nguyên tố và xu hướng hóa học trong giao diện học tập trực quan.</p>
          <p className="mt-2 text-xs text-outline">Route: {appRoutes.periodicTable}</p>

          <div className="mt-5 flex flex-wrap gap-3">
            <Link href={appRoutes.learningPathway} className="rounded-full border-2 border-on-primary-fixed-variant bg-primary px-5 py-2 font-label-sm text-on-primary shadow-[0_4px_0_0_rgba(0,67,149,0.3)]">
              Mở lộ trình liên quan
            </Link>
            <Link href={appRoutes.experimentLibrary} className="rounded-full border-2 border-outline-variant bg-surface px-5 py-2 font-label-sm text-on-surface hover:bg-surface-container">
              Mở thư viện thí nghiệm
            </Link>
